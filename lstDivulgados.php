...
$sql  = "select pet.id, pet.nome, pet.idade, pet.sexo, raca.raca, pet.porte, animal.tipoanimal, pet.descricao, pet.adotador, pet.imagem, usuario.login, usuario.username, usuario.telefone from pet  inner join usuario on pet.adotador = usuario.id
...
<body class="teal">
  ...
  <div class="had-container center">
                            ...
                            <div class="row login">
                            <h1 class="center">Pets Divulgados</h1> 
                            <br>                         
        ...
        <div class="card card1">
          ...
          <div class="card-content">
            <span class="card-title activator grey-text text-darken-4"><?php echo $adocao['nome'] ?><i class="material-icons right">more_vert</i></span>
               <?php if ($adocao['adotador']==0){ ?>
                <br>
                <button class="btn waves-effect waves-light teal darken-3" type="button" id="btnVoltar" onclick="JavaScript:location.href='frmEdtPet.php?id=<?php echo $adocao['id']?>'">
                                            Editar
                                        </button>
                                        <button class="btn waves-effect waves-light teal darken-3" type="button" id="btnVoltar" onclick="JavaScript:location.href='frmRemPet.php?id=<?php echo $adocao['id']?>'">
                                            Excluir
                                        </button>
               <?php } 
               else{?>
                <span>O Pet foi adotado</span><br>
                <span>Adotador: <?php echo $adocao['username'] ?></span><br>
          ...
          <br>
            ...
            <span>Porte: <?php if($adocao['porte']=='P'){?>Pequeno<?php }else if($adocao['porte']=='M'){?>Médio<?php }else{?>Grande<?php }?></span></h5>
          </div>
        </div>
      </div>
  <?php
